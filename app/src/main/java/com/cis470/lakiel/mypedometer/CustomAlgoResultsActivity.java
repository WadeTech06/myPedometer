...
public class CustomAlgoResultsActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int wPeak = getIntent().getIntExtra("wPeak", 18);
        int jPeak = getIntent().getIntExtra("jPeak", 25);
        int rPeak = getIntent().getIntExtra("rPeak", 32);


        Intent stepsAnalysisIntent = new Intent(getApplicationContext(), StepsTrackerService.class);
        stepsAnalysisIntent.putExtra("wPeak",wPeak);
        stepsAnalysisIntent.putExtra("jPeak",jPeak);
        stepsAnalysisIntent.putExtra("rPeak",rPeak);
